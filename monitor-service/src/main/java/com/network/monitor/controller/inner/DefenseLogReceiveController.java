...
public class DefenseLogReceiveController {
    ...
    public ApiResponse<Void> syncDefenseLog(@RequestBody DefenseLogDTO logDTO) {
        try {
            log.info("接收到防御日志：defenseType={}, defenseTarget={}, executeStatus={}", 
                logDTO.getDefenseType(), logDTO.getDefenseTarget(), logDTO.getExecuteStatus());
            ...
            return ApiResponse.error("处理防御日志失败：" + e.getMessage());
        }
    }
}
